#!/bin/bash
# A2 消融: 数据均衡的影响
# balanced (FVQA-train) vs unbalanced (全量数据)
# 论文 Finding 5: 去掉 balancing → 搜索率飙到 ~100%
#
# 运行: bash mmsearch_r1/scripts/run_ablation_A2.sh 1   # balanced
#       bash mmsearch_r1/scripts/run_ablation_A2.sh 0   # unbalanced
...
BALANCED=${1:-1}
if [ "$BALANCED" = "1" ]; then
    WANDB_EXP_NAME="ablation_A2_balanced"
    TRAIN_FILE=$TRAIN_DATA_PATH
else
    WANDB_EXP_NAME="ablation_A2_unbalanced"
    TRAIN_FILE=${UNBALANCED_DATA_PATH:-$TRAIN_DATA_PATH}
fi
