#!/bin/bash
# A3 消融: SFT 冷启动的影响
# 无 SFT (base model) vs SFT+GRPO
#
# 运行: bash mmsearch_r1/scripts/run_ablation_A3.sh 0   # 无 SFT
#       bash mmsearch_r1/scripts/run_ablation_A3.sh 1   # SFT+GRPO
...
USE_SFT=${1:-0}
if [ "$USE_SFT" = "1" ]; then
    WANDB_EXP_NAME="ablation_A3_sft_grpo"
    MODEL_PATH=${SFT_CHECKPOINT_PATH:-checkpoints/mmsearch-r1/sft_tool_classifier}
else
    WANDB_EXP_NAME="ablation_A3_no_sft_grpo"
